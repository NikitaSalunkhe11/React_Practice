        ...
        <div>
            ...
            {storeData.map((product)=>(
              <div style={{backgroundColor:"lightblue", margin:"20px", width:"400px", boxShadow:"2px 2px 10px grey"}}>
                <h3 style={{margin:"20px"}}>{product.title}</h3>
                <p style={{color:"red", fontSize:"20px", margin:"20px"}}>{product.price}</p>
                <button 
                style={{marginLeft:"50px", height:"30px", width:"150px", backgroundColor:"lightgreen", margin:"20px"}} 
                onClick={()=>removeProductdispatch(removeProduct(product.id))}>Remove Product</button>
              </div>
            ))}
            </div>
        </div>
